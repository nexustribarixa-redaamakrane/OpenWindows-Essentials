/*
 * squashfsdrv.c - OpenWindows SquashFS Compressed Read-Only Driver (.owc)
 *
...
#include "squashfsdrv.h"

static squashfsdrv_state_t g_squashfsdrv_state;
...
{
    if (!state) { state = &g_squashfsdrv_state; }
...
bool squashfsdrv_stat(squashfsdrv_state_t *state, const char *path, ow_vfs_stat_t *out_stat)
{
    ...
    out_stat->fs_type = OW_FS_TYPE_SQUASHFS;
...
static bool vfs_mount_adapter(ow_vfs_mount_t *vmount, const void *boot_sector)
{
    ...
    return squashfsdrv_mount(&g_squashfsdrv_state, boot_sector);
}

static bool vfs_unmount_adapter(ow_vfs_mount_t *vmount)
{
    if (!vmount) { return false; }
    g_squashfsdrv_state.is_mounted = false;
    return true;
}
...
{
    (void)vmount;
    int32_t h = squashfsdrv_open(&g_squashfsdrv_state, path, flags);
    if (h >= 0 && out_file) {
...
static bool vfs_close_adapter(ow_vfs_file_t *file)
{
    ...
    file->is_open = false;
    return squashfsdrv_close(&g_squashfsdrv_state, (int32_t)file->handle_id);
}
...
static size_t vfs_read_adapter(ow_vfs_file_t *file, uint64_t offset, void *buffer, size_t bytes_to_read)
{
    ...
    return squashfsdrv_read(&g_squashfsdrv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_read);
}

static size_t vfs_write_adapter(ow_vfs_file_t *file, uint64_t offset, const void *buffer, size_t bytes_to_write)
{
    if (!file || !file->is_open) { return 0u; }
    return squashfsdrv_write(&g_squashfsdrv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_write);
}

static bool vfs_stat_adapter(ow_vfs_mount_t *vmount, const char *path, ow_vfs_stat_t *out_stat)
{
    (void)vmount;
    return squashfsdrv_stat(&g_squashfsdrv_state, path, out_stat);
}
...
static const ow_vfs_ops_t g_squashfsdrv_vfs_ops = {
