/*
 * ext4drv.c - OpenWindows Linux ext2/ext3/ext4 Filesystem Driver (.owc)
 *
...
static ext4drv_state_t g_ext4drv_state;
...
{
    if (!state) { state = &g_ext4drv_state; }
...
bool ext4drv_stat(ext4drv_state_t *state, const char *path, ow_vfs_stat_t *out_stat)
{
    ...
    out_stat->fs_type = OW_FS_TYPE_EXT4;
...
static bool vfs_mount_adapter(ow_vfs_mount_t *vmount, const void *boot_sector)
{
    ...
    return ext4drv_mount(&g_ext4drv_state, boot_sector);
}

static bool vfs_unmount_adapter(ow_vfs_mount_t *vmount)
{
    if (!vmount) { return false; }
    g_ext4drv_state.is_mounted = false;
    return true;
}
...
{
    (void)vmount;
    int32_t h = ext4drv_open(&g_ext4drv_state, path, flags);
    if (h >= 0 && out_file) {
...
static bool vfs_close_adapter(ow_vfs_file_t *file)
{
    ...
    file->is_open = false;
    return ext4drv_close(&g_ext4drv_state, (int32_t)file->handle_id);
}
...
static size_t vfs_read_adapter(ow_vfs_file_t *file, uint64_t offset, void *buffer, size_t bytes_to_read)
{
    ...
    return ext4drv_read(&g_ext4drv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_read);
}

static size_t vfs_write_adapter(ow_vfs_file_t *file, uint64_t offset, const void *buffer, size_t bytes_to_write)
{
    if (!file || !file->is_open) { return 0u; }
    return ext4drv_write(&g_ext4drv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_write);
}

static bool vfs_stat_adapter(ow_vfs_mount_t *vmount, const char *path, ow_vfs_stat_t *out_stat)
{
    (void)vmount;
    return ext4drv_stat(&g_ext4drv_state, path, out_stat);
}
...
static const ow_vfs_ops_t g_ext4drv_vfs_ops = {
