/*
 * cdfsdrv.c - OpenWindows Compact Disc File System (CDFS / ISO 9660) Driver (.owc)
 *
...
static cdfsdrv_state_t g_cdfsdrv_state;
...
{
    if (!state) { state = &g_cdfsdrv_state; }
...
bool cdfsdrv_stat(cdfsdrv_state_t *state, const char *path, ow_vfs_stat_t *out_stat)
{
    ...
    out_stat->fs_type = OW_FS_TYPE_ISO9660;
...
static bool vfs_mount_adapter(ow_vfs_mount_t *vmount, const void *boot_sector)
{
    ...
    return cdfsdrv_mount(&g_cdfsdrv_state, boot_sector);
}

static bool vfs_unmount_adapter(ow_vfs_mount_t *vmount)
{
    if (!vmount) { return false; }
    g_cdfsdrv_state.is_mounted = false;
    return true;
}
...
{
    (void)vmount;
    int32_t h = cdfsdrv_open(&g_cdfsdrv_state, path, flags);
    if (h >= 0 && out_file) {
...
static bool vfs_close_adapter(ow_vfs_file_t *file)
{
    ...
    file->is_open = false;
    return cdfsdrv_close(&g_cdfsdrv_state, (int32_t)file->handle_id);
}
...
static size_t vfs_read_adapter(ow_vfs_file_t *file, uint64_t offset, void *buffer, size_t bytes_to_read)
{
    ...
    return cdfsdrv_read(&g_cdfsdrv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_read);
}

static size_t vfs_write_adapter(ow_vfs_file_t *file, uint64_t offset, const void *buffer, size_t bytes_to_write)
{
    if (!file || !file->is_open) { return 0u; }
    return cdfsdrv_write(&g_cdfsdrv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_write);
}

static bool vfs_stat_adapter(ow_vfs_mount_t *vmount, const char *path, ow_vfs_stat_t *out_stat)
{
    (void)vmount;
    return cdfsdrv_stat(&g_cdfsdrv_state, path, out_stat);
}
...
static const ow_vfs_ops_t g_cdfsdrv_vfs_ops = {
