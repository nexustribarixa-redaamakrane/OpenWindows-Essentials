/*
 * f2fsdrv.c - OpenWindows Flash-Friendly File System (F2FS) Driver (.owc)
 *
...
static f2fsdrv_state_t g_f2fsdrv_state;
...
{
    if (!state) { state = &g_f2fsdrv_state; }
...
bool f2fsdrv_stat(f2fsdrv_state_t *state, const char *path, ow_vfs_stat_t *out_stat)
{
    ...
    out_stat->fs_type = OW_FS_TYPE_F2FS;
...
static bool vfs_mount_adapter(ow_vfs_mount_t *vmount, const void *boot_sector)
{
    ...
    return f2fsdrv_mount(&g_f2fsdrv_state, boot_sector);
}

static bool vfs_unmount_adapter(ow_vfs_mount_t *vmount)
{
    if (!vmount) { return false; }
    g_f2fsdrv_state.is_mounted = false;
    return true;
}
...
{
    (void)vmount;
    int32_t h = f2fsdrv_open(&g_f2fsdrv_state, path, flags);
    if (h >= 0 && out_file) {
...
static bool vfs_close_adapter(ow_vfs_file_t *file)
{
    ...
    file->is_open = false;
    return f2fsdrv_close(&g_f2fsdrv_state, (int32_t)file->handle_id);
}
...
static size_t vfs_read_adapter(ow_vfs_file_t *file, uint64_t offset, void *buffer, size_t bytes_to_read)
{
    ...
    return f2fsdrv_read(&g_f2fsdrv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_read);
}

static size_t vfs_write_adapter(ow_vfs_file_t *file, uint64_t offset, const void *buffer, size_t bytes_to_write)
{
    if (!file || !file->is_open) { return 0u; }
    return f2fsdrv_write(&g_f2fsdrv_state, (int32_t)file->handle_id, offset, buffer, bytes_to_write);
}

static bool vfs_stat_adapter(ow_vfs_mount_t *vmount, const char *path, ow_vfs_stat_t *out_stat)
{
    (void)vmount;
    return f2fsdrv_stat(&g_f2fsdrv_state, path, out_stat);
}
...
static const ow_vfs_ops_t g_f2fsdrv_vfs_ops = {
